...
WITH v1 AS (
    ...
    WHERE (d_year = 2000 OR
           (d_year = 1999 AND d_moy = 12) OR
           (d_year = 2001 AND d_moy = 1))
    GROUP BY i_category, i_brand, cc_name, d_year, d_moy
),
v2 AS (
    SELECT v1.cc_name, v1.d_year, v1.d_moy,
           v1.avg_monthly_sales,
           v1.sum_sales, v1_lag.sum_sales AS psum, v1_lead.sum_sales AS nsum
    FROM v1
    LEFT JOIN v1 v1_lag ON v1.i_category = v1_lag.i_category
                         AND v1.i_brand = v1_lag.i_brand
                         AND v1.cc_name = v1_lag.cc_name
                         AND v1.rn = v1_lag.rn + 1
    LEFT JOIN v1 v1_lead ON v1.i_category = v1_lead.i_category
